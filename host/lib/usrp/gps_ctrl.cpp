...
private:
    ...
    // Normalize GN-prefixed NMEA strings to GP and fix checksum when needed
    std::string normalize_nmea_prefix(std::string nmea)
    {
        if (not _is_mp) {
            return nmea;
        }
        if (nmea.size() < 6 || nmea[0] != '$') {
            return nmea;
        }
        if (nmea[1] != 'G' || nmea[2] != 'N') {
            return nmea;
        }
        const auto star_pos = nmea.find('*');
        if (star_pos == std::string::npos || star_pos + 2 > nmea.size()) {
            return nmea;
        }
        ...
        for (size_t i = 1; i < star_pos; i++) {
            crc ^= static_cast<uint8_t>(nmea[i]);
        }
        std::stringstream ss;
        ss << std::uppercase << std::hex << std::setw(2) << std::setfill('0')
           << static_cast<int>(crc);
        nmea.replace(star_pos + 1, 2, ss.str());
        return nmea;
    }
    ...
    void update_cache(const std::string& msg_key_hint = "")
    {
        ...
        for (std::string msg = _recv(0); not msg.empty(); msg = _recv(0)) {
            ...
            // Normalize message (GN->GP) if requested
            std::string norm_msg = normalize_nmea_prefix(msg);

            // Look for SERVO message
            if (std::regex_search(
                    norm_msg, servo_regex, std::regex_constants::match_continuous)) {
                UHD_LOG_TRACE("GPS", "Received new SERVO message: " << msg);
                msgs["SERVO"] = norm_msg;
            } else if (std::regex_match(norm_msg, gp_msg_regex)
                and is_nmea_checksum_ok(norm_msg)) {
                UHD_LOG_TRACE("GPS",
                    "Received new " << norm_msg.substr(1, 5) << " message: " << norm_msg);
                msgs[norm_msg.substr(1, 5)] = norm_msg;
            } else {
